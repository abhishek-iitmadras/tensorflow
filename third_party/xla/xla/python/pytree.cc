...
SequenceKey::SequenceKey(nb::kwargs kwargs) {
  if (kwargs.contains("idx")) {
    idx_ = nanobind::cast<int>(kwargs["idx"]);
  } else {
    throw std::invalid_argument("SequenceKey must have an `idx` argument.");
  }
}

DictKey::DictKey(nb::kwargs kwargs) {
  if (kwargs.contains("key")) {
    key_ = kwargs["key"];
  } else {
    throw std::invalid_argument("DictKey must have an `key` argument.");
  }
}

GetAttrKey::GetAttrKey(nb::kwargs kwargs) {
  if (kwargs.contains("name")) {
    name_ = nb::str(kwargs["name"]);
  } else {
    throw std::invalid_argument("GetAttrKey must have an `name` argument.");
  }
}
...
void BuildPytreeSubmodule(nb::module_& m) {
  ...
  sequence_key.def(nb::init<int>());
  sequence_key.def(nb::init<nb::kwargs>());
  ...
  sequence_key.def("__hash__", [](const SequenceKey& key) {
    return nb::hash(nb::str(absl::StrCat("SequenceKey ", key.idx()).c_str()));
  });
  ...
  dict_key.def(nb::init<nb::object>());
  dict_key.def(nb::init<nb::kwargs>());
  ...
  get_attr_key.def(nb::init<nanobind::str>());
  get_attr_key.def(nb::init<nb::kwargs>());
  ...
  nb::class_<FlattenedIndexKey> flattened_index_key(pytree,
                                                    "FlattenedIndexKey");
  flattened_index_key.def(nb::init<int>());
  ...
  flattened_index_key.def("__hash__", [](const FlattenedIndexKey& key) {
    return nb::hash(
        nb::str(absl::StrCat("FlattenedIndexKey ", key.key()).c_str()));
  });
